...
best_sedi_score = -np.inf  # En yüksek SEDI skoru
best_sedi_params = None
best_sedi_model = None
    ...
    for max_depth in param_grid['max_depth']:
        for min_samples_split in param_grid['min_samples_split']:
            for min_samples_leaf in param_grid['min_samples_leaf']:
                ...
                results.append({'params': rf.get_params(), 'sedi': SEDI})

                # Eğer bu modelin SEDI skoru en iyi skordan daha iyiyse, en iyi skoru ve modeli güncelle
                if SEDI > best_sedi_score:
                    best_sedi_score = SEDI
                    best_sedi_params = {'n_estimators': n_estimators,
                                        'max_depth': max_depth,
                                        'min_samples_split': min_samples_split,
                                        'min_samples_leaf': min_samples_leaf}
                    best_sedi_model = rf  # En iyi modeli güncelle

                # Eğer SEDI skoru 0.85'den büyükse, modeli kaydet
                if SEDI > 0.78:
                    model_filename = f'model_s{SEDI}_n{n_estimators}_d{max_depth}_s{min_samples_split}_l{min_samples_leaf}.joblib'
                    dump(rf, model_filename)
                    print(f"SEDI {SEDI:.2f} olan model '{model_filename}' dosyasına kaydedildi.")

# En iyi modelin kaydedilmesi
if best_sedi_model is not None:
    dump(best_sedi_model, 'best_model_sedi.joblib')
    print(f"Optimize edilmiş model 'best_model_sedi.joblib' dosyasına kaydedildi.")

# Sonuçları CSV dosyasına yazma
results_df = pd.DataFrame(results)
results_df.to_csv('predictions_vs_y_test.csv', index=False)

print("CSV dosyası oluşturuldu: 'predictions_vs_y_test.csv'")

"""


# Modeli yükleme
loaded_model = load('model_s0.7983252259007685_n200_d5_s20_l4.joblib')
